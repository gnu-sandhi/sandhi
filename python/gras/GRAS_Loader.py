...
def __module_import(p):
    ...
    for sub in os.listdir(p):
        ...
        if ext == ".py" and (has_pyc or has_pyo):
            continue
        if ext == ".pyc" and has_pyo:
            continue
...
__load_modules_from_paths(os.getenv("GRAS_ROOT", "@GRAS_ROOT@"), os.path.join(
    "lib@LIB_SUFFIX@", "gras", "python"))
__load_modules_from_paths(os.getenv("GRAS_PATH", ""),
                          os.path.join("lib@LIB_SUFFIX@", "gras", "python"))
