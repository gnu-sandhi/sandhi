...
class TestNumericArrayTypes(unittest.TestCase):
    ...
    def test_ints_loopback(self):
        for dtype in (numpy.int8, numpy.int16, numpy.int32):
            for nums in (1, 10, 100):
                print 'dtype', dtype, 'nums', nums
                bits = numpy.dtype(dtype).itemsize * 8
                if bits == 32:
                    bits = 31  # avoid OverflowError: Python int too large to convert to C long
                cap = (1 << bits) - 1
                print '=' * 10, 'Test dtype %s, num bits %u, nitems %u' % (dtype, bits, nums)
                a0 = numpy.array(
                    numpy.random.randint(-cap / 2, +cap / 2, nums), dtype)
                self.loopback(a0)
