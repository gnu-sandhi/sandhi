...
class test_freq_xlating_filter(gr_unittest.TestCase):
    ...
    def generate_ccf_source(self):
        ...
        times = xrange(100)
        self.src_data = map(
            lambda t: cmath.exp(-2j * cmath.pi * fc / fs * (t / 100.0)), times)
    ...
    def generate_fcf_source(self):
        ...
        self.taps = filter.firdes.low_pass(1, fs, bw, bw / 4)
        times = xrange(100)
        self.src_data = map(lambda t: math.sin(
    ...
    def generate_scf_source(self):
        ...
        self.taps = filter.firdes.low_pass(1, fs, bw, bw / 4)
        times = xrange(100)
        self.src_data = map(lambda t: int(
            100 * math.sin(2 * cmath.pi * fc / fs * (t / 100.0))), times)
    ...
    def test_fir_filter_ccf_001(self):
        ...
        self.assertComplexTuplesAlmostEqual(
            expected_data, result_data[-20:], 5)
