...
def _from_file(xml):
    ...
    tag = xml.tag
    if not len(xml):
        # store empty tags (text is None) as empty string
        return odict({tag: xml.text or ''})
    nested_data = odict()
    for elem in xml:
        key, value = _from_file(elem).items()[0]
        if nested_data.has_key(key):
...
def to_file(nested_data, xml_file):
    ...
    open(xml_file, 'w').write(etree.tostring(
        xml, xml_declaration=True, pretty_print=True))
