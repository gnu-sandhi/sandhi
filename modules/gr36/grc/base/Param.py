...
def _get_elem(lst, key):
    try:
        ...
        raise ValueError, 'Key "%s" not found in %s.' % (key, _get_keys(lst))
...
class Option(Element):
    ...
    def __str__(self): return 'Option %s(%s)' % (
...
class Param(Element):
    ...
    def validate(self):
        ...
        if self.get_type() not in self.get_types():
            self.add_error_message(
                'Type "%s" is not a possible type.' % self.get_type())
    ...
    def get_hide(self): return self.get_parent(
    ).resolve_dependencies(self._hide).strip()
    ...
    def get_type(self): return self.get_parent(
    ).resolve_dependencies(self._type)
    ...
    # Access Opts
    ##############################################
    def get_opt_keys(self): return self.get_option(
        self.get_value()).get_opt_keys()

    def get_opt(self, key): return self.get_option(
