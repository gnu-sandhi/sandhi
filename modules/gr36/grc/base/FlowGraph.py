...
class FlowGraph(Element):
    ...
    def __str__(self): return 'FlowGraph - %s(%s)' % (
    ...
    # Access Elements
    ##############################################
    def get_block(self, id): return filter(
        lambda b: b.get_id() == id, self.get_blocks())[0]

    def get_blocks(self): return filter(
        lambda e: e.is_block(), self.get_elements())

    def get_connections(self): return filter(
        lambda e: e.is_connection(), self.get_elements())
    ...
    def connect(self, porta, portb):
        ...
        """
        connection = self.get_parent().Connection(
    ...
    def export_data(self):
        ...
        n['connection'] = [connection.export_data()
        ...
        for block_n in blocks_n:
            ...
            if block:
                ...
                Messages.send_error_load(
                    'Block key "%s" not found in %s' % (key, self.get_parent()))
        ...
        for connection_n in connections_n:
            ...
            try:
                ...
                if source_block_id not in block_ids:
                    raise LookupError(
                        'source block id "%s" not in block ids' % source_block_id)
                if sink_block_id not in block_ids:
                    raise LookupError(
                        'sink block id "%s" not in block ids' % sink_block_id)
                # get the blocks
                ...
                if source_key not in source_block.get_source_keys():
                    raise LookupError(
                        'source key "%s" not in source block keys' % source_key)
                if sink_key not in sink_block.get_sink_keys():
                    raise LookupError(
                        'sink key "%s" not in sink block keys' % sink_key)
                # get the ports
