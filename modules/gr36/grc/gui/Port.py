...
class Port(Element):
    ...
    def create_shapes(self):
        ...
        if self.is_source():
            ports = self.get_parent().get_sources()
        elif self.is_sink():
            ports = self.get_parent().get_sinks()
        # get the max width
    ...
    def create_labels(self):
        ...
        self.W, self.H = 2 * PORT_LABEL_PADDING + \
            self.w, 2 * PORT_LABEL_PADDING + self.h
        # create the pixmap
    ...
    def draw(self, gc, window):
        ...
        # use the first area's sizes to place the labels
        (x, y), (w, h) = self._areas_list[0]
        if self.is_horizontal():
            window.draw_drawable(gc, self.horizontal_label, 0, 0, x + X +
                                 (self.W - self.w) / 2, y + Y + (self.H - self.h) / 2, -1, -1)
        elif self.is_vertical():
            window.draw_drawable(gc, self.vertical_label, 0, 0, x + X +
                                 (self.H - self.h) / 2, y + Y + (self.W - self.w) / 2, -1, -1)
