...
class ActionHandler:
    ...
    def _handle_action(self, action):
        ...
        ##################################################
            ...
            for block in self.get_flow_graph().get_selected_blocks():
                ...
                for param in block.get_params():
                    for variable in self.get_flow_graph().get_variables():
                        # If a block parameter exists that is a variable,
                    ...
                    for flow_param in self.get_flow_graph().get_parameters():
                        # If a block parameter exists that is a parameter,
                        # create a parameter for it
                ...
                for connection in block.get_connections():
                    ...
                    # If connected block is not in the list of selected blocks
                    # create a pad for it
                    if self.get_flow_graph().get_block(source_id) not in self.get_flow_graph().get_selected_blocks():
                        pads.append({'key': connection.get_sink().get_key(), 'coord': connection.get_source(
                        ).get_coordinate(), 'block_id': block.get_id(), 'direction': 'source'})

                    if self.get_flow_graph().get_block(sink_id) not in self.get_flow_graph().get_selected_blocks():
                        pads.append({'key': connection.get_source().get_key(), 'coord': connection.get_sink(
                        ).get_coordinate(), 'block_id': block.get_id(), 'direction': 'sink'})
            ...
            for pad in pads:
                ...
                if pad['direction'] == 'sink':
                    ...
                    source_block = self.get_flow_graph(
                    ).get_block(pad['block_id'])
                    source = source_block.get_source(pad['key'])

                    # Ensure the port types match
                    while pad_sink.get_type() != source.get_type():

                        # Special case for some blocks that have non-standard
                        # type names, e.g. uhd
                    ...
                    sink_block = self.get_flow_graph(
                    ).get_block(pad['block_id'])
        ...
        Actions.ERRORS_WINDOW_DISPLAY.set_sensitive(
            not self.get_flow_graph().is_valid())
        Actions.ELEMENT_DELETE.set_sensitive(
            bool(self.get_flow_graph().get_selected_elements()))
        Actions.BLOCK_PARAM_MODIFY.set_sensitive(
            bool(self.get_flow_graph().get_selected_block()))
        Actions.BLOCK_ROTATE_CCW.set_sensitive(
            bool(self.get_flow_graph().get_selected_blocks()))
        Actions.BLOCK_ROTATE_CW.set_sensitive(
            bool(self.get_flow_graph().get_selected_blocks()))
        # update cut/copy/paste
        Actions.BLOCK_CUT.set_sensitive(
            bool(self.get_flow_graph().get_selected_blocks()))
        Actions.BLOCK_COPY.set_sensitive(
            bool(self.get_flow_graph().get_selected_blocks()))
        Actions.BLOCK_PASTE.set_sensitive(bool(self.clipboard))
        # update enable/disable
        Actions.BLOCK_ENABLE.set_sensitive(
            bool(self.get_flow_graph().get_selected_blocks()))
        Actions.BLOCK_DISABLE.set_sensitive(
            bool(self.get_flow_graph().get_selected_blocks()))
        Actions.BLOCK_CREATE_HIER.set_sensitive(
            bool(self.get_flow_graph().get_selected_blocks()))
        Actions.OPEN_HIER.set_sensitive(
            bool(self.get_flow_graph().get_selected_blocks()))
    ...
    def update_exec_stop(self):
        ...
        Actions.FLOW_GRAPH_KILL.set_sensitive(
            self.get_page().get_proc() != None)
