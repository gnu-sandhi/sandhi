...
class BlockTreeWindow(gtk.VBox):
    ...
    def __init__(self, platform, get_flow_graph):
        ...
        self.treeview.enable_model_drag_source(
            gtk.gdk.BUTTON1_MASK, DND_TARGETS, gtk.gdk.ACTION_COPY)
    ...
    def add_block(self, category, block=None):
        ...
        self.treestore.set_value(iter, DOC_INDEX, Utils.parse_template(
            DOC_MARKUP_TMPL, doc=block.get_doc()))
    ...
    def _handle_search(self, model, column, key, iter):
        ...
        matching_blocks = filter(
            lambda b: key in b.get_key() or key in b.get_name().lower(), blocks)
        # remove the old search category
        try:
            self.treestore.remove(
                self._categories.pop((self._search_category, )))
        ...
        for block in matching_blocks:
            self.add_block(self._search_category, block)
        # expand the search category
        path = self.treestore.get_path(
            self._categories[(self._search_category, )])
    ...
    def _handle_drag_get_data(self, widget, drag_context, selection_data, info, time):
        ...
        key = self._get_selected_block_key()
        if key:
            selection_data.set(selection_data.target, 8, key)
