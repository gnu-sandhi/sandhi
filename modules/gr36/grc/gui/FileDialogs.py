...
class FileDialogHelper(gtk.FileChooserDialog):
    ...
    def __init__(self, action, title):
        ...
        gtk.FileChooserDialog.__init__(
            self, title, None, action, ('gtk-cancel', gtk.RESPONSE_CANCEL, ok_stock, gtk.RESPONSE_OK))
    ...
    def __init__(self, current_file_path=''):
        ...
        if not current_file_path:
            current_file_path = path.join(
                DEFAULT_FILE_PATH, NEW_FLOGRAPH_TITLE + Preferences.file_extension())
        if self.type == OPEN_FLOW_GRAPH:
            FileDialogHelper.__init__(
                self, gtk.FILE_CHOOSER_ACTION_OPEN, 'Open a Flow Graph from a File...')
            self.add_and_set_filter(get_flow_graph_files_filter())
            self.set_select_multiple(True)
        elif self.type == SAVE_FLOW_GRAPH:
            FileDialogHelper.__init__(
                self, gtk.FILE_CHOOSER_ACTION_SAVE, 'Save a Flow Graph to a File...')
            self.add_and_set_filter(get_flow_graph_files_filter())
            # show the current filename
            self.set_current_name(path.basename(current_file_path))
        elif self.type == SAVE_IMAGE:
            FileDialogHelper.__init__(
                self, gtk.FILE_CHOOSER_ACTION_SAVE, 'Save a Flow Graph Screen Shot...')
            ...
            if path.exists(filename):  # ask the user to confirm overwrite
                if MessageDialogHelper(
                    ...
                    Utils.parse_template(
                        FILE_OVERWRITE_MARKUP_TMPL, filename=filename),
                ) == gtk.RESPONSE_NO:
            ...
            for filename in filenames:
                if not path.exists(filename):  # show a warning and re-run
                    MessageDialogHelper(
                        ...
                        Utils.parse_template(
                            FILE_DNE_MARKUP_TMPL, filename=filename),
                    )
                    return self.get_rectified_filename()
            return filenames
