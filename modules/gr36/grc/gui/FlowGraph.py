...
class FlowGraph(Element):
    ...
    def new_pixmap(
        self, *args): return self.get_drawing_area().new_pixmap(*args)
        ...
        if coor is None:
            coor = (
                int(random.uniform(.25, .75) *
                    h_adj.page_size + h_adj.get_value()),
                int(random.uniform(.25, .75) *
                    v_adj.page_size + v_adj.get_value()),
            )
        # get the new block
    ...
    def paste_from_clipboard(self, clipboard):
        ...
        for connection_n in connections_n:
            source = old_id2block[connection_n.find('source_block_id')].get_source(
                connection_n.find('source_key'))
            sink = old_id2block[connection_n.find('sink_block_id')].get_sink(
                connection_n.find('sink_key'))
    ...
    def update_selected_elements(self):
        ...
        if self.mouse_pressed:
            ...
            if self.get_ctrl_mask() or not (
                new_selections and new_selections[
                    0] in self.get_selected_elements()
            ):
