...
class Block(Element):
    ...
    def create_labels(self):
        ...
        self._bg_color = self.get_enabled(
        ) and Colors.BLOCK_ENABLED_COLOR or Colors.BLOCK_DISABLED_COLOR
        layouts = list()
        ...
        # display the params
        markups = [param.get_markup() for param in self.get_params()
        ...
        self.H = max(*(
            [self.label_height + 2 * BLOCK_LABEL_PADDING] + [2 * PORT_BORDER_SEPARATION +
                                                             sum([
                                                                 port.H + PORT_SEPARATION for port in ports]) - PORT_SEPARATION
    ...
    def draw(self, gc, window):
        ...
        # draw label image
        if self.is_horizontal():
            window.draw_drawable(gc, self.horizontal_label, 0, 0, x +
                                 BLOCK_LABEL_PADDING, y + (self.H - self.label_height) / 2, -1, -1)
        elif self.is_vertical():
            window.draw_drawable(gc, self.vertical_label, 0, 0, x +
                                 (self.H - self.label_height) / 2, y + BLOCK_LABEL_PADDING, -1, -1)
        # draw ports
