...
class InputParam(gtk.HBox):
    ...
    def _update_gui(self, *args):
        ...
        self.set_markup(Utils.parse_template(
            PARAM_LABEL_MARKUP_TMPL, param=self.param, has_cb=has_cb))
        # set the color
...
class EntryParam(InputParam):
    ...
    def set_color(self, color):
        ...
        self._input.modify_text(
            gtk.STATE_NORMAL, Colors.PARAM_ENTRY_TEXT_COLOR)
...
class EnumParam(InputParam):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._input.set_active(
            self.param.get_option_keys().index(self.param.get_value()))
        self._input.connect('changed', self._handle_changed)
        self.pack_start(self._input, False)

    def get_text(self): return self.param.get_option_keys()[
        self._input.get_active()]
...
class EnumEntryParam(InputParam):
    ...
    def set_color(self, color):
        if self._input.get_active() == -1:  # custom entry, use color
            ...
            self._input.get_child().modify_text(
                gtk.STATE_NORMAL, Colors.PARAM_ENTRY_TEXT_COLOR)
        else:  # from enum, make pale background
            self._input.get_child().modify_base(
                gtk.STATE_NORMAL, Colors.ENTRYENUM_CUSTOM_COLOR)
            self._input.get_child().modify_text(
                gtk.STATE_NORMAL, Colors.PARAM_ENTRY_TEXT_COLOR)
