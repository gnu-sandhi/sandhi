...
def _prefs_file(): return os.path.join(
    os.path.expanduser('~'), file_extension())
...
def files_open(files=None):
    if files is not None:
        ...
        while True:
            try:
                files.append(_config_parser.get(
                    'files_open', 'file_open_%d' % i))
            except:
                return files
            i = i + 1
...
def reports_window_position(pos=None):
    if pos is not None:
        ...
        try:
            # greater than 0
            return _config_parser.getint('main', 'reports_window_position') or 1
        except:
            return -1
...
def blocks_window_position(pos=None):
    if pos is not None:
        ...
        try:
            # greater than 0
            return _config_parser.getint('main', 'blocks_window_position') or 1
        except:
            return -1
