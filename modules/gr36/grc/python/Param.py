...
ID_BLACKLIST = ['self', 'options', 'gr', 'blks2', 'wxgui', 'wx', 'math', 'forms', 'firdes'] + \
    filter(lambda x: not x.startswith('_'), dir(
        gr.top_block())) + dir(__builtin__)
        ...
        def num_to_str(num):
            if isinstance(num, COMPLEX_TYPES):
                ...
                if num == 0:
                    ...
                    # value is real
                    return '%s' % eng_notation.num_to_str(num.real)
                elif num.real == 0:
                    # value is imaginary
                    return '%sj' % eng_notation.num_to_str(num.imag)
                elif num.imag < 0:
                    return '%s-%sj' % (eng_notation.num_to_str(num.real), eng_notation.num_to_str(abs(num.imag)))
                else:
                    return '%s+%sj' % (eng_notation.num_to_str(num.real), eng_notation.num_to_str(num.imag))
            else:
                return str(num)
        ##################################################
    ...
    def evaluate(self):
        ...
        #########################
            ...
            try:
                ...
                raise Exception, 'Value "%s" cannot be evaluated:\n%s' % (v, e)
            # raise an exception if the data is invalid
            if t == 'raw':
                return e
            elif t == 'complex':
                if not isinstance(e, COMPLEX_TYPES):
                    raise Exception, 'Expression "%s" is invalid for type complex.' % str(
                        e)
                return e
            elif t == 'real':
                if not isinstance(e, REAL_TYPES):
                    raise Exception, 'Expression "%s" is invalid for type real.' % str(
                        e)
                return e
            elif t == 'int':
                if not isinstance(e, INT_TYPES):
                    raise Exception, 'Expression "%s" is invalid for type integer.' % str(
                        e)
                return e
            elif t == 'hex':
                return hex(e)
                ...
                if not isinstance(e, bool):
                    raise Exception, 'Expression "%s" is invalid for type bool.' % str(
                        e)
                return e
            else:
                raise TypeError, 'Type "%s" not handled' % t
        #########################
        # Numeric Vector Types
        #########################
        elif t in ('complex_vector', 'real_vector', 'int_vector'):
            if not v:
                v = '()'  # turn a blank string into an empty list, so it will eval
            # raise exception if python cannot evaluate this value
            try:
                e = self.get_parent().get_parent().evaluate(v)
            except Exception, e:
            ...
            if t == 'complex_vector':
                ...
                if not all([isinstance(ei, COMPLEX_TYPES) for ei in e]):
                    raise Exception, 'Expression "%s" is invalid for type complex vector.' % str(
                        e)
                return e
            elif t == 'real_vector':
                if not isinstance(e, VECTOR_TYPES):
                ...
                if not all([isinstance(ei, REAL_TYPES) for ei in e]):
                    raise Exception, 'Expression "%s" is invalid for type real vector.' % str(
                        e)
                return e
            elif t == 'int_vector':
                if not isinstance(e, VECTOR_TYPES):
                ...
                if not all([isinstance(ei, INT_TYPES) for ei in e]):
                    raise Exception, 'Expression "%s" is invalid for type integer vector.' % str(
                        e)
                return e
        #########################
        # String Types
        ...
        #########################
            ...
            def gui_hint(ws, w):
                if 'layout' in w:
                    ws = ws.replace('addWidget', 'addLayout')
                return ws % w
        ...
        #########################
            ...
            if not v:
                return ''  # allow for empty grid pos
            e = self.get_parent().get_parent().evaluate(v)
            ...
            for param in params:
                for parent, cell in param._hostage_cells:
                    if (parent, cell) in self._hostage_cells:
                        raise Exception, 'Another graphical element is using parent "%s", cell "%s".' % (
                            str(parent), str(cell))
            return e
        #########################
        # Notebook Page Type
        #########################
            ...
            if not v:
                return ''  # allow for empty notebook
            ...
            try:
                notebook_block = filter(
                    lambda b: b.get_id() == notebook_id, notebook_blocks)[0]
            except:
                raise Exception, 'Notebook id "%s" is not an existing notebook id.' % notebook_id
        ...
        #########################
            ...
            try:
                ...
                raise Exception, 'Import "%s" failed.' % v
            except Exception:
                raise Exception, 'Bad import syntax: "%s".' % v
