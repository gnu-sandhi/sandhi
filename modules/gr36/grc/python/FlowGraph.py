...
class FlowGraph(_FlowGraph, _GUIFlowGraph):
    ...
    # we only want stream ports
        sorted_pads = filter(lambda b: b.get_param(
            'type').get_evaluated() != 'message', sorted_pads)
        # load io signature
        return [{
    ...
    def get_imports(self):
        ...
        """
        imports = sum([block.get_imports()
                       for block in self.get_enabled_blocks()], [])
    ...
    def get_variables(self):
        ...
        """
        variables = filter(lambda b: _variable_matcher.match(
            b.get_key()), self.get_enabled_blocks())
    ...
    def get_parameters(self):
        ...
        """
        parameters = filter(lambda b: _parameter_matcher.match(
            b.get_key()), self.get_enabled_blocks())
        return parameters
