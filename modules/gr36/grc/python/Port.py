...
def _get_source_from_virtual_sink_port(vsp):
    ...
    try:
        ...
        raise Exception, 'Could not resolve source for virtual sink port %s' % vsp
...
def _get_source_from_virtual_source_port(vsp, traversed=[]):
    ...
    if not vsp.get_parent().is_virtual_source():
        return vsp
    if vsp in traversed:
        raise Exception, 'Loop found when resolving virtual source %s' % vsp
    try:
        return _get_source_from_virtual_source_port(
...
def _get_sink_from_virtual_sink_port(vsp, traversed=[]):
    ...
    if not vsp.get_parent().is_virtual_sink():
        return vsp
    if vsp in traversed:
        raise Exception, 'Loop found when resolving virtual sink %s' % vsp
    try:
        return _get_sink_from_virtual_sink_port(
        ...
        if self.get_type() == 'msg':
            if self.get_nports():
                self.add_error_message(
                    'A port of type "msg" cannot have "nports" set.')
            if self.get_vlen() != 1:
                self.add_error_message(
                    'A port of type "msg" must have a "vlen" of 1.')
