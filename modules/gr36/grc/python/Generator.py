...
Add a Misc->Throttle block to your flow graph to avoid CPU congestion.''')
    ...
    def get_popen(self):
        ...
        # -u is unbuffered stdio
        cmds = [python_exe, '-u', self.get_file_path()]
    ...
    def __str__(self):
        ...
        """
        title = self._flow_graph.get_option(
            'title') or self._flow_graph.get_option('id').replace('_', ' ').title()
        ...
        def _get_block_sort_text(block):
            ...
            try:
                # older gui markup w/ wxgui
                code += block.get_param('notebook').get_value()
            except:
                pass
            try:
        ...
        connections = filter(lambda c: not (
            c.is_msg() or c.is_message()), self._flow_graph.get_enabled_connections())
        ...
        # prepend self.
        replace_dict = dict([(var_id, 'self.%s' % var_id)
