...
def convert_hier(flow_graph, python_file):
    ...
    block_name = flow_graph.get_option(
        'title') or flow_graph.get_option('id').replace('_', ' ').title()
    ...
    if parameters:
        block_n['make'] = '%s(\n\t%s,\n)' % (
            block_key,
            ',\n\t'.join(['%s=$%s' % (param.get_id(), param.get_id())
                          for param in parameters]),
        )
    else:
        block_n['make'] = '%s()' % block_key
    # callback data
    block_n['callback'] = ['set_%s($%s)' % (
        param.get_id(), param.get_id()) for param in parameters]
    # param data
    params_n = list()
    for param in parameters:
        param_n = odict()
        param_n['name'] = param.get_param(
            'label').get_value() or param.get_id()
