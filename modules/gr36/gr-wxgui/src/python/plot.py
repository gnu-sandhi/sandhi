...
class PolyMarker(PolyPoints):
    ...
    def _circle(self, dc, coords, size=1):
        fact = 2.5 * size
        wh = 5.0 * size
        rect = _numpy.zeros((len(coords), 4), _numpy.float) + \
            [0.0, 0.0, wh, wh]
...
class PlotCanvas(wx.Window):
    ...
    def Draw(self, graphics, xAxis=None, yAxis=None, dc=None, step=None):
        ...
        if xAxis == None or yAxis == None:
            ...
            # lower left corner user scale (xmin,ymin)
            p1 = _numpy.array([xAxis[0], yAxis[0]])
            # upper right corner user scale (xmax,ymax)
            p2 = _numpy.array([xAxis[1], yAxis[1]])
        ...
        textSize_scale = _numpy.array([rhsW + lhsW, bottomH + topH])
        # shift plot area by this amount
        textSize_shift = _numpy.array([lhsW, bottomH])
        ...
        if graphics.getYLabel():  # bug fix for Linux
            dc.DrawRotatedText(graphics.getYLabel(),
                               yLabelPos[0], yLabelPos[1], 90)

        # drawing legend makers and text
        if self._legendEnabled:
            self._drawLegend(dc, graphics, rhsW, topH,
                             legendBoxWH, legendSymExt, legendTextExt)

        # allow for scaling and shifting plotted points
        scale = (self.plotbox_size - textSize_scale) / \
            (p2 - p1) * _numpy.array((1, -1))
        shift = -p1 * scale + self.plotbox_origin + \
            textSize_shift * _numpy.array((1, -1))
        ...
        if self.use_persistence:
            ...
            if True:
                ...
                _numpy.multiply(self._Buffer3array,
                                alpha_int, self._Buffer3array)
                _numpy.multiply(self._Buffer2array,
                                (256 - alpha_int), self._Buffer2array)
                _numpy.add(self._Buffer3array,
                           self._Buffer2array, self._Buffer2array)
                _numpy.right_shift(self._Buffer2array, 8, self._Buffer2array)
            elif False:
                self._Buffer2array = (self._Bufferarray.astype(
                    _numpy.uint32) * alpha_int + self._Buffer2array * (256 - alpha_int)).__rshift__(8)
            elif False:
                self._Buffer2array *= (256 - alpha_int)
                self._Buffer2array += self._Bufferarray.astype(
                    _numpy.uint32) * alpha_int
                self._Buffer2array /= 256

            # copy back to image buffer
            # , format=wx.BitmapBufferFormat_RGB, stride=-1)
            self._Buffer2.CopyFromBuffer(
                self._Buffer2array.astype(_numpy.uint8))
    ...
    def OnMouseLeftUp(self, event):
        if self._zoomEnabled:
            if self._hasDragged == True:
                ...
                minX, minY = _numpy.minimum(
                    self._zoomCorner1, self._zoomCorner2)
                maxX, maxY = _numpy.maximum(
                    self._zoomCorner1, self._zoomCorner2)
                self.last_PointLabel = None  # reset pointLabel
                if self.last_draw != None:
                    self.Draw(self.last_draw[0], xAxis=(
                        minX, maxX), yAxis=(minY, maxY), dc=None)
    ...
    def OnSize(self, event):
        ...
        if True:  # self.use_persistence:
            ...
            self._Bufferarray = _numpy.zeros(
                (Size[0] * Size[1] * 3), dtype=_numpy.uint8)
            ...
            self._Buffer2array = _numpy.zeros(
                (Size[0] * Size[1] * 3), dtype=_numpy.uint32)  # dtype=_numpy.float
            self._Buffer3array = _numpy.zeros(
                (Size[0] * Size[1] * 3), dtype=_numpy.uint32)  # dtype=_numpy.float
...
def _draw2Objects():
    ...
    data1.shape = (100, 2)
    data1[:, 1] = _numpy.sin(data1[:, 0])
    line1 = PolyLine(data1, legend='Green Line',
                     colour='green', width=6, style=wx.DOT)

    # 50 points cos function, plotted as red dot-dash
    data1 = 2. * _numpy.pi * _numpy.arange(100) / 100.
    data1.shape = (50, 2)
    data1[:, 1] = _numpy.cos(data1[:, 0])
    line2 = PolyLine(data1, legend='Red Line', colour='red',
                     width=3, style=wx.DOT_DASH)
...
class TestFrame(wx.Frame):
    ...
    def OnMotion(self, event):
        ...
        if self.client.GetEnablePointLabel() == True:
            ...
            dlst = self.client.GetClosetPoint(
                self.client.GetXY(event), pointScaled=True)
