...
# A dictionary of the number of lines to delete from the beginning of
# docstrings
lines_to_delete = {}
...
class OldBlockDocumenter(FunctionDocumenter):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if len(bits) != 3 or bits[0] != 'gnuradio':
            raise ValueError(
                "expected name to be of form gnuradio.x.y but it is {0}".format(self.name))
        sptr_name = 'gnuradio.{0}.{0}_{1}_sptr'.format(bits[1], bits[2])
        # Create a Class Documenter to create documentation for the classes
        # members.
        self.classdoccer = ClassDocumenter(
